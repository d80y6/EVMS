...
const DIRECTIONS = ['up-left', 'up', 'up-right', 'left', 'right', 'down-left', 'down', 'down-right'] as const;
...
export default function PtzOverlay({ cameraId, visible, onVisibilityChange }: PtzOverlayProps) {
  ...
  const handlePresetGoto = useCallback(async (presetToken: string) => {
            ...
            {DIRECTIONS.map((dir) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center justify-center text-white/70 hover:text-white hover:bg-white/10 rounded disabled:opacity-50 text-sm transition-colors"
